...
window.Documentation = {
	...
	*/
	initModIndex : function(){
		...
		if(nodes.length){
			0 && console.log('need to fix initModIndex');
		}
		...
		/* original code
		 var togglers = $('img.toggler').click(function(){
			...
			0 && console.log($('tr.cg-' + idnum).toggle());
...
window.Search = {
	...
	*/
	...
	query : function(inQuery){
		...
		0 && console.debug('SEARCH: searching for:');
		0 && console.info('required: ', searchterms);
		0 && console.info('excluded: ', excluded);
		// prepare search
...
define("docs/CodeGlassViewerMini", [
...
define("docs/RstEditor", [
	...
	return declare(null, [_WidgetBase, _HasDropDown],{
		...
		postCreate: function(){
			...
			0 && console.log(this.toggleButton, this.saveButton,this.resetButton)
		...
		commit: function(message){
			...
			0 && console.log('editing: ', this.editing);
			if(this._previousContent && this.domNode.value != this._previousContent){
				this.saving = true;
				this.editorMessage.innerHTML = "Saving.";
		...
		getPreview:function(force){
			...
			if(!this.previewPanel.innerHTML || !this._previewContent || this.domNode.value != this._previewContent){
				0 && console.log("posting: ", { preview: this.domNode.value });
