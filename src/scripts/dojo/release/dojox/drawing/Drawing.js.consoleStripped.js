			...
			dojo.forEach(this.plugins, function(p, i){
				...
				try{
					this.plugins[i] = new this.tools[p.name](props);
					 ...
					 0 && console.error("Failed to initilaize plugin:	" +p.name + ". Did you require it?");
				}
			}, this);
			this.plugins = [];
		...
		setTool: function(/* String */type){
			...
			try{
				...
				this.currentStencil.connect(this.currentStencil, "destroy", this, "onDeleteStencil");
				 ...
				 0 && console.error("dojox.drawing.setTool Error:", e);
				 0 && console.error(this.currentType + " is not a constructor: ", this.tools[this.currentType]);
				// 0 && console.trace();
			}
		},
