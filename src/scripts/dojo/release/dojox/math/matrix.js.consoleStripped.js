...
dojo.mixin(dojox.math.matrix, {
	...
	multiply: function(/* Array */a, /* Array */b){
		...
		if(ax!=by){
			 0 && console.warn("Can't multiply matricies of sizes " + ax + "," + ay + " and " + bx + "," + by);
			return [[0]];
		}
		var c=[];
		for (var k=0; k<ay; k++) {
	...
	product: function(/* Array... */){
		...
		if (arguments.length==0){
			 0 && console.warn("can't multiply 0 matrices!");
			return 1;
		}
		var m=arguments[0];
	...
	sum: function(/* Array... */){
		...
		if(arguments.length==0){
			 0 && console.warn("can't sum 0 matrices!");
			return 0;	// Number
		}
		var m=this.copy(arguments[0]);
		var rows=m.length;
		if(rows==0){
			 0 && console.warn("can't deal with matrices of 0 rows!");
			return 0;
		}
		var cols=m[0].length;
		if(cols==0){
			 0 && console.warn("can't deal with matrices of 0 cols!");
			return 0;
		}
		for(var i=1; i<arguments.length; ++i){
			var arg=arguments[i];
			if(arg.length!=rows || arg[0].length!=cols){
				 0 && console.warn("can't add matrices of different dimensions: first dimensions were " + rows + "x" + cols + ", current dimensions are " + arg.length + "x" + arg[0].length);
				return 0;
			}
	...
	inverse: function(/* Array */a){
		...
		if(det==0){
			 0 && console.warn("Determinant Equals 0, Not Invertible.");
			return [[0]];
		}else{
			dd=1/det;
		}
	...
	determinant: function(/* Array */a){
		...
		if(a.length!=a[0].length){
			 0 && console.warn("Can't calculate the determinant of a non-squre matrix!");
			return 0;
		}
	...
	upperTriangle: function(/* Array */m){
		...
		for(var col=0; col<tms-1; col++){
			if(typeof m[col][col]!="number") {
				 0 && console.warn("non-numeric entry found in a numeric matrix: m[" + col + "][" + col + "]=" + m[col][col]);
			}
			v=1;
			var stop_loop=0;
			...
			for(var row=col+1; row<tms; row++){
				if(typeof m[row][col]!="number"){
					 0 && console.warn("non-numeric entry found in a numeric matrix: m[" + row + "][" + col + "]=" + m[row][col]);
				}
				if(typeof m[col][row]!="number"){
					 0 && console.warn("non-numeric entry found in a numeric matrix: m[" + col + "][" + row + "]=" + m[col][row]);
				}
				if(m[col][col]!=0){
	...
	adjoint: function(/* Array */a){
		...
		var tms=a.length;
		if(tms<=1){
			 0 && console.warn("Can't find the adjoint of a matrix with a dimension less than 2");
			return [[0]];
		}
		if(a.length!=a[0].length){
			 0 && console.warn("Can't find the adjoint of a non-square matrix");
			return [[0]];
		}
