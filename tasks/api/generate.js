    ...
    getDetails(detailsfile, modulename, version, function (err, pageObject) {
        process.nextTick(function () {
            if (err || !pageObject) {
				...
				retObjectItem.modulename = modulename;
                ...
                // usage - handle both constructors and "top level functions" (though these impart less info)
                // e.g. dojo/NodeList, dojo/on etc
                if ((pageObject.type === "constructor" && pageObject.methods) || pageObject.type === "function") {
                    var isTopLevelFunction = (pageObject.type === "function");
                    ...
                    if (methodItem !== null && (typeof methodItem !== 'undefined')) {
                        ...
                        retObjectItem.usage.isTopLevelFunction = isTopLevelFunction;
                        retObjectItem.usage.from =  methodItem.from;
                        retObjectItem.usage.returnTypes = methodItem.returnTypes;
                        retObjectItem.usage.returnDescription = methodItem.returnDescription;
                        var params = [];
                        if (methodItem.parameters) { //TODO this should really be methodItem.usage.parameters
                            pageObject.parameters.forEach(function (item) {
                                params.push(item.name);
                            });
                        }
                        retObjectItem.usage.parameterList = params; // allow empty array
                        if (!isTopLevelFunction) { // TODO, should probably clean this logic up
                            if (methodItem.description) {
                                retObjectItem.usage.description = methodItem.description; // allow null
                            } else if (methodItem.summary) {
                                retObjectItem.usage.summary = methodItem.summary; // allow null
                            }
                        } else {
                            retObjectItem.usage.summary = methodItem.summary; // allow null
                        }
                ...
                if (modulename.match(/^(.*\.|)__/)) {
                    retObjectItem.kwArgs = true;
                }
                // end kwargs

                // generate properties
                // todo:
                var shortSummary, inherited = null;
                if (pageObject.properties) {
                    ...
                    pageObject.properties.forEach(function (item) {
                        ...
                        // see dijit/_TemplatedMixin.searchContainerNode
                        if (item.from !== pageObject.location && !item.extensionModule) {
                            inherited = true;
                        }
                    ...
                    retObjectItem.properties = lclProps.sort(compareItemsNames); // allow empty, sort possibly not needed now
                }
...
function getMethodOrEventObjects(methods, pageObject) {
    ...
    methods.forEach(function (item) {
        if (item.name !== 'constructor') { // constructors never appear as methods, they're described in the module summary
